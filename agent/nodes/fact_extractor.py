"""Node 3: Fact Extractor — extracts structured facts using Groq/Llama.

Uses Groq (Llama 3.3 70B) for fast, cost-effective structured extraction.
Groq's speed advantage makes it ideal for processing multiple search results
in rapid succession without high latency.
"""
...
def fact_extractor(state: AgentState) -> dict:
    ...
    Processes the most recent search results (not previously processed)
    and outputs subject-predicate-object triples with confidence scores.
    """
    llm = ChatGroq(
    ...
    # Get results from the most recent search batch
    ...
    # Process results in chunks to stay within context limits
    ...
    try:
        ...
        log_msg = (
            f"[{datetime.now().isoformat()}] FactExtractor: "
        ...
        return {
            "execution_log": [f"[{datetime.now().isoformat()}] FactExtractor: FAILED — {e}"],
        }
